...
const FIX_DIR = join(OUTPUT_DIR, 'Medicine-or-Fix');
const reportLines = [];
const results = [];
const sitemapLinks = [];
...
function checkRootAndFiles() {
  ...
  const required = [
    ...
    'web/admin/guide/index.html', 'worker/package.json', 'worker/src/index.ts', 'worker/wrangler.toml', 'worker/schema.sql', 'scripts/rinova-doctor.mjs', 'Doctor-report/README.md', 'Doctor-report/Medicine-or-Fix/README.md',
...
async function checkCloudflareResources() {
  ...
  const secretGroups = [{ label: 'Admin username', names: ['ADMIN_USERNAME'], required: true }, { label: 'Admin password', names: ['ADMIN_PASSWORD'], required: true }, { label: 'Admin automation token', names: ['ADMIN_API_TOKEN'] }, { label: 'Steadfast key pair', names: ['STEADFAST_API_KEY', 'STEADFAST_SECRET_KEY'] }, { label: 'Steadfast webhook token', names: ['STEADFAST_WEBHOOK_TOKEN'] }, { label: 'Gemini fallback', names: ['GEMINI_API_KEY', 'GEMINI_API_KEY_1', 'GEMINI_API_KEY_2'] }];
...
function writeReports() {
  ...
  const audit = ['# Rinova BD Doctor Audit Report', '', `- **Overall:** ${overall}`, `- **Started:** ${startedAt}`, `- **Repository root:** \`${rel(ROOT)}\``, `- **Target website:** \`${TARGET_URL}\``, `- **Report directory:** \`${rel(OUTPUT_DIR)}\``, '- **Mode:** Read-only. No customer rows, product rows, orders, KV values or secret values were printed or changed.', '', '| Result | Area | Finding | Root file / location | Developer fix |', '|---|---|---|---|---|', ...reportLines, '', '## Sitemap link summary', '', `- **Total URLs found:** ${sitemapLinks.length}`, `- **Reachable:** ${sitemapLinks.filter((item) => item.ok).length}`, `- **Needs attention:** ${sitemapLinks.filter((item) => !item.ok).length}`, '', 'See `sitemap-links.md` for every URL and HTTP result.', '', '## How to use this report', '', 'Open `Medicine-or-Fix/fix-report.md` for only the WARN/FAIL items. Each item includes a repository-relative root file, line-area reference, problem, required change and verification step. Line numbers refer to the checked-out commit and should be rechecked after edits.'].join('\n');
  ...
  const fixReport = ['# Medicine-or-Fix Report', '', `- **Audit:** \`${rel(OUTPUT_DIR)}\``, `- **Overall:** ${overall}`, '- **Secret policy:** Names and set/missing status only; secret values are never written.', '', fixSections, ''].join('\n');
  const summary = { overall, counts, targetUrl: TARGET_URL, readOnly: true, startedAt, reportDirectory: rel(OUTPUT_DIR), files: ['audit-report.md', 'sitemap-links.md', 'summary.json', 'Medicine-or-Fix/fix-report.md'], sitemap: { total: sitemapLinks.length, reachable: sitemapLinks.filter((item) => item.ok).length, needsAttention: sitemapLinks.filter((item) => !item.ok) .length } };
