...
/**
 * The product's own offer, set in the product editor. The server sends the percentage and the
 * already-discounted price; this applies the same percentage to a chosen size, because a size
 * carries its own list price and the offer has to come off that too.
 */
function offerPercent() { return Math.max(0, Math.round(Number(state.offerPercent || 0))); }
function afterOffer(price) {
  const base = Math.max(0, Math.round(Number(price) || 0));
  const percent = offerPercent();
  return percent ? Math.max(0, Math.round((base * (100 - percent)) / 100)) : base;
}

function variantFor(label) {
  return state.variants.find((variant) => variant.kind === 'size' && String(variant.label).toLowerCase() === String(label).toLowerCase());
}
/** The price the shop set for one size, after any offer, or null when it uses the base price. */
function variantPriceFor(label) {
  const match = variantFor(label);
  return match && match.price !== null && match.price !== undefined ? afterOffer(match.price) : null;
}
/** A shop counts stock per size only once it has entered some; all zeroes means "not tracked". */
function tracksSizeStock() { return state.variants.some((variant) => variant.kind === 'size' && Number(variant.stock || 0) > 0); }
function sizeSoldOut(label) { return tracksSizeStock() && Number(variantFor(label)?.stock || 0) <= 0; }
...
function productOptionsMarkup(product) {
  ...
  const sizeOption = (size) => { const price = variantPriceFor(size); const out = sizeSoldOut(size); return `<option value="${escapeHtml(size)}" ${out ? 'disabled' : ''}>${escapeHtml(size)}${price !== null ? ` — ${money(price)}` : ''}${out ? ' — sold out' : ''}</option>`; };
...
function render(product, ratingSummary, reviews, slug) { state.media = parseMedia(product); state.mediaIndex = 0; const adminPreview = new URLSearchParams(window.location.search).get('admin_preview') === '1' && Boolean(sessionStorage.getItem('rinova-admin-token')); const adminEditLink = adminPreview ? `<a class="admin-preview-edit" target="_parent" href="/admin/?view=products&edit=${encodeURIComponent(product.id)}">Edit this product in Admin Dashboard ${icon('arrowRight')}</a>` : ''; const tiers = (() => { try { return JSON.parse(product.volumeTiersJson || '[]'); } catch { return []; } })(); const tierText = tiers.length ? `<div class="detail-note"><strong>Volume pricing available.</strong><br>${tiers.map((tier) => `${tier.minQty}+ units · ${money(tier.price)} each`).join(' · ')}</div>` : ''; const faqAnswer = escapeHtml(product.shortDescription || product.description || 'Please follow the instructions on the product packaging and choose what feels right for your routine.');
  ...
  const pricedSizes = state.variants.filter((variant) => variant.kind === 'size' && variant.price !== null && variant.price !== undefined).map((variant) => afterOffer(variant.price));
  ...
  const applySizePrice = () => {
    ...
    if (!node) return;
    // The struck-through price follows the size, so a discounted 100g shows what the 100g was.
    const was = $('#detail-price-was');
    const showWas = (now, before) => { if (!was) return; was.hidden = !(before > now); was.textContent = money(before); };
    if (price !== null) { node.textContent = money(price); showWas(price, offerPercent() ? Number(variantFor(chosen)?.price || 0) : 0); return; }
    if (lowest !== null && (!listPrice || lowest < salePrice)) { node.textContent = `From ${money(lowest)}`; showWas(lowest, 0); return; }
    node.textContent = money(salePrice);
    showWas(salePrice, Number(product.wasPrice || product.compareAtPrice || 0));
  };
  $('#detail-size')?.addEventListener('change', applySizePrice);
  applySizePrice();
  // Whatever goes into the bag is the price on screen, which is the price the server charges.
  const priced = () => ({ ...product, price: state.activePrice === null ? (salePrice || lowest || 0) : state.activePrice });
