...
function orderCard(order) {
  const itemSummary = (order.items || []).map((item) => `${escapeHtml(item.productName)} <span data-rinova-icon="close"></span> ${Number(item.quantity || 0)}`).join(' · ') || 'Product details unavailable';
