...
The doctor runs from the checked-out repository root and validates repository paths, the CI/CD workflow markers, workspace/toolchain scripts, Wrangler D1/KV/Workers AI/assets bindings, migration numbering and key schema files, Cloudflare account/D1/KV/Worker/R2 visibility, public storefront endpoints, protected API boundaries, and obvious hard-coded secret assignments. The diagnostic is **read-only**: it does not create, update or delete products, orders, customers, KV values, D1 rows, buckets, Workers, or secrets. The workflow separately commits only the generated redacted report files so developers can see them in GitHub. It reports only whether secrets are set or missing and never prints secret values, customer rows, order rows or database contents.

When the run finishes, open the generated **rinova-doctor-report** artifact or browse the same published folder in the repository at `Doctor-report/runs/run-<run-number>-<run-id>/`. The workflow commits that folder after every manual run, so it is not artifact-only. Each run contains `audit-report.md`, `sitemap-links.md`, `summary.json`, and `Medicine-or-fixd/fix-report.md`. `audit-report.md` covers the whole project; `sitemap-links.md` lists every sitemap URL with its individual HTTP result; and `Medicine-or-fixd/fix-report.md` gives the root file, line-area reference, problem, required change and verification step for every WARN/FAIL. `PASS` means the contract was observed, `WARN` means an optional integration or credential is not configured, and `FAIL` means developer action is required. The current expected warning is optional R2 until the account enables it and binds `PRODUCT_IMAGES`; a correctly configured deployment should show zero `FAIL` findings.
